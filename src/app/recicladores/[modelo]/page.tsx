...
const db: RecicladoresDatabase = require("@/db.json");
...
export default function Page({ params }: { params: { modelo: string } }) {
  ...
  if (!db.recicladores.modelos.hasOwnProperty(modelo)) {
    return <div className="mt-24">Modelo não encontrado</div>;
  }

  const modeloData = db.recicladores.modelos[modelo];
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      productName: `Reciclador ${modeloData.modelName} - Item ${inputValue} - ${modelValue}`,
      quantity: 1,
    };
            ...
            {modeloData.items?.length > 0 &&
              modeloData.items.map((item: any) => (
                <li key={item.archiveName}>
                  <Image
                    src={`/img/recicladores/${modelo}/${item?.archiveName}`}
                    width={1200}
                    height={700}
                    alt={item.itemName}
