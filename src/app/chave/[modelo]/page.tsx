...
const db: chaveDatabase = require("@/db.json");
...
export default function Page({ params }: { params: { modelo: string } }) {
  ...
  if (!db.chave.modelos.hasOwnProperty(modelo)) {
    return <div className="mt-24">Modelo não encontrado</div>;
  }

  const modeloData = db.chave.modelos[modelo];
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      productName: `Chave de Impacto ${modeloData.modelName} - Item ${inputValue} - ${modelValue}`,
      quantity: 1,
    };
            ...
            {modeloData.items?.length > 0 &&
              modeloData.items.map((item: any) => (
                <li key={item.archiveName}>
                  <Image
                    src={`/img/chave/${modelo}/${item?.archiveName}`}
                    width={1200}
                    height={700}
                    alt={item.itemName}
