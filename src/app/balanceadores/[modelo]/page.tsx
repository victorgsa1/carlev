...
export default function Page({ params }: { params: { modelo: string } }) {
  ...
  if (!db.balanceadores.modelos.hasOwnProperty(modelo)) {
    return <div className="mt-24">Modelo não encontrado</div>;
  }

  const modeloData = db.balanceadores.modelos[modelo];
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      productName: `Balanceador ${modeloData.modelName} - Item ${inputValue} - ${modelValue}`,
      quantity: 1,
    };
