...
const db: RampaDatabase = require("@/db.json");
...
export default function Page({ params }: { params: { modelo: string } }) {
  ...
  if (!db.rampa.modelos.hasOwnProperty(modelo)) {
    return <div className="mt-24">Modelo não encontrado</div>;
  }

  const modeloData = db.rampa.modelos[modelo];
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      productName: `Rampa ${modeloData.modelName} - Item ${inputValue} - ${modelValue}`,
      quantity: 1,
    };
            ...
            {modeloData.items?.length > 0 &&
              modeloData.items.map((item: any) => (
                <li key={item.archiveName}>
                  <Image
                    src={`/img/rampa/${modelo}/${item?.archiveName}`}
                    width={1200}
                    height={700}
                    alt={item.itemName}
